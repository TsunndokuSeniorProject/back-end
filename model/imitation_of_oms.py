...
def preProcessing(text_review):
    cachedStopWords = nltk.corpus.stopwords.words("english")
    cachedStopWords.append('OMG')
    cachedStopWords.append(':-)')
    result=(' '.join([word for word in text_review.split() if word not in cachedStopWords]))
    return str(result)

def tokenizeReviews(text_review):
    tokenizedReviews={}
    tokenizer = nltk.tokenize.punkt.PunktSentenceTokenizer()
    uniqueId=1
    cachedStopWords = nltk.corpus.stopwords.words("english")
    for sentence in tokenizer.tokenize(text_review):      
        tokenizedReviews[uniqueId]=sentence
        uniqueId+=1
    return tokenizedReviews

def posTagging(text_review):
    outputPost={}
    for key,value in text_review.items():
        outputPost[key]=nltk.pos_tag(nltk.word_tokenize(value))
    return outputPost

def aspectExtraction(dict_pos_token):
    prevWord=''
    prevTag=''
    currWord=''
    aspectList=[]
    outputDict={}
    #Extracting Aspects
    for key,value in dict_pos_token.items():
...
def identifyOpinionWords(outputPost, outputAspect):       
    ...
    for aspect,no in outputAspect:
        ...
        if(count>0):
            outputAspectOpinionTuples[aspect][0]=round((outputAspectOpinionTuples[aspect][0]/float(count))*100,2)
            outputAspectOpinionTuples[aspect][1]=round((outputAspectOpinionTuples[aspect][1]/float(count))*100,2)
            outputAspectOpinionTuples[aspect][2]=round((outputAspectOpinionTuples[aspect][2]/float(count))*100,2)
            print(aspect,': Positive => ', outputAspectOpinionTuples[aspect][0], ' Negative => ',outputAspectOpinionTuples[aspect][1])
            opinion_result.append({
                "aspect":aspect,
                "positive":outputAspectOpinionTuples[aspect][0],
                "negative":outputAspectOpinionTuples[aspect][1]
            })
...
if __name__ == "__main__":
    ...
    b = tokenizeReviews(a)
    c = posTagging(b)
    d = aspectExtraction(c)
    from pprint import pprint
    pprint(identifyOpinionWords(c, d))
