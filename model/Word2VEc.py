...
def test_model(data_path, vocab_size):
    ...
    model.load_weights("C:/Users/USER/Back-end/word2vec_weights.h5")
    word_target, word_context = zip(*couples)
    word_target = np.array(word_target, dtype="int32")
    word_context = np.array(word_context, dtype="int32")
    result = model.evaluate([word_target, word_context], labels)
    print(model.metrics_names)
    print(result)
    embedding_layer = model.get_layer("embedding")
    word_vector = embedding_layer.get_weights()[0]
    word_embeddings = {word:word_vector[idx] for word, idx in tokenizer.word_index.items()}
...
def train_model(data_path, vocab_size, num_epochs):
    ...
    word_target = np.array(word_target, dtype="int32")
    word_context = np.array(word_context, dtype="int32")
    history = model.fit([word_target, word_context], labels, epochs=num_epochs, callbacks=[history], validation_split=0.20)
